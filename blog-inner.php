<?php
/*
Template Name: Blog-inner Page
*/
?>
<?php get_header(); ?>

    <section class="s-inner" style="background-image: url(<? bloginfo('template_url' ); ?>/img/expertise-bg.jpg);">
        <div class="container">
            <div class="s-inner-content">
                <h1 class="h1">Someone has a ignorance and zero tolerance</h1>
                <div class="item-row">
                    <div class="item-wrap">
                        <div class="inner-blog__avatar">
                            <img src="<? bloginfo('template_url' ); ?>/img/blog-avatar.png" alt="alt">
                        </div>
                        <span class="inner-blog__author">by John Smith</span>
                    </div>
                    <div class="item-wrap">
                        <div class="inner-blog__date">28 May, 2018</div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    
    <section class="inner-blog">
        <div class="container">
            <div class="s-ui-content">
                <div class="inner-blog__top">
                    <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.  Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.  Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.  </p>
                </div>
            </div>
            <div class="inner-blog__img">
                <img src="<? bloginfo('template_url' ); ?>/img/ask-case.jpg" alt="alt">
            </div>
            <div class="inner-blog-content">
                <div class="inner-blog__wrap">
                    <div class="inner-blog__top">
                        <h3 class="h3">ABC Corparation</h3>
                        <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. </p>
                        <div class="s-review-block">
                            <blockquote>“Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s”</blockquote>
                        </div>
                    </div>
                </div>
                <div class="inner-blog__bottom">
                    <div class="row justify-content-between">
                        <div class="col-auto">
                            <div class="inner-blog-info">
                                <span class="head first">Share this article</span>
                                <ul class="socials socials--custom">
                                    <li>
                                        <a href="#" class="facebook" target="_blank">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="10" height="20" viewBox="0 0 10 20"><defs><path id="7xqya" d="M1279.67 6754.67v-1.99c0-.9.2-1.35 1.59-1.35h1.74V6748h-2.91c-3.57 0-4.75 1.64-4.75 4.44v2.23H1273v3.33h2.34v10h4.33v-10h2.93l.4-3.33z"/></defs><g><g transform="translate(-1273 -6748)"><use xlink:href="#7xqya"/></g></g></svg>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#" class="twitter" target="_blank">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="22.26" height="18.09" viewBox="0 0 22.26 18.09"><defs><path id="xusna" d="M1320 6768.1c8.4 0 13-6.97 13-13 0-.2 0-.4-.02-.6a9.29 9.29 0 0 0 2.28-2.36 9 9 0 0 1-2.63.72 4.58 4.58 0 0 0 2.01-2.53c-.9.53-1.88.9-2.9 1.1a4.57 4.57 0 0 0-7.78 4.18 12.97 12.97 0 0 1-9.41-4.77 4.57 4.57 0 0 0 1.41 6.1 4.5 4.5 0 0 1-2.07-.58v.06a4.57 4.57 0 0 0 3.66 4.47c-.67.19-1.37.21-2.06.08a4.57 4.57 0 0 0 4.27 3.17 9.19 9.19 0 0 1-6.76 1.9 12.92 12.92 0 0 0 7 2.05"/></defs><g><g transform="translate(-1313 -6750)"><use xlink:href="#xusna"/></g></g></svg>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="#" class="linkedin" target="_blank">
                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="22.26" height="20" viewBox="0 0 22.26 20"><defs><path id="52zma" d="M1369.57 6747.6c1.47 0 2.67 1.08 2.67 2.4 0 1.34-1.2 2.42-2.67 2.42-1.49 0-2.68-1.08-2.68-2.41s1.2-2.41 2.68-2.41zm-2.32 6.64h4.62v13.36h-4.62z"/><path id="52zmb" d="M1374.77 6754.23h4.42v1.83h.06c.62-1.05 2.12-2.15 4.37-2.15 4.66 0 5.53 2.76 5.53 6.36v7.32h-4.6v-6.5c0-1.54-.03-3.54-2.4-3.54-2.4 0-2.76 1.7-2.76 3.43v6.61h-4.61l-.01-13.36z"/></defs><g><g transform="translate(-1366.89 -6747.6)"><g><use fill="#999" xlink:href="#52zma"/></g><g><use xlink:href="#52zmb"/></g></g></g></svg>
                                        </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-auto">
                            <div class="inner-blog-info">
                               <span class="head">Written by</span>
                               <div class="wrap">
                                   <div class="inner-blog__avatar custom">
                                        <img src="<? bloginfo('template_url' ); ?>/img/blog-avatar.png" alt="alt">
                                    </div>
                                    <span class="inner-blog__by">by John Smith</span>
                               </div>
                            </div>
                        </div>
                        <div class="col-md-auto">
                            <div class="inner-blog-info">
                               <span class="head third">Date of Creation</span> 
                               <div class="content">28 May, 2018 at 17:50</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="s-blog">
        <div class="container">
            <h2 class="h2">What esle do we have</h2>
            <div class="s-blog-block">
                <div class="s-slider-nav blog">
                    <span class="s-slider-nav__prev blog__prev">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="57" height="57" viewBox="0 0 57 57"><defs><path id="up7sa" d="M209 4154.5a28.5 28.5 0 1 1 57 0 28.5 28.5 0 0 1-57 0z"/><path id="up7sb" d="M243.7 4153.7l-7.2-7.2a1.7 1.7 0 0 0-2.4 0l-.6.6a1.7 1.7 0 0 0 0 2.4l5.5 5.5-5.5 5.5a1.7 1.7 0 0 0 0 2.4l.6.6a1.7 1.7 0 0 0 2.4 0l7.2-7.2c.36-.36.52-.83.5-1.3.02-.47-.14-.94-.5-1.3z"/></defs><g><g class="round" transform="translate(-209 -4126)"><use xlink:href="#up7sa"/></g><g class="arrow" transform="translate(-209 -4126)"><use xlink:href="#up7sb"/></g></g></svg>
                    </span>
                    <span class="s-slider-nav__next blog__next">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="57" height="57" viewBox="0 0 57 57"><defs><path id="up7sa" d="M209 4154.5a28.5 28.5 0 1 1 57 0 28.5 28.5 0 0 1-57 0z"/><path id="up7sb" d="M243.7 4153.7l-7.2-7.2a1.7 1.7 0 0 0-2.4 0l-.6.6a1.7 1.7 0 0 0 0 2.4l5.5 5.5-5.5 5.5a1.7 1.7 0 0 0 0 2.4l.6.6a1.7 1.7 0 0 0 2.4 0l7.2-7.2c.36-.36.52-.83.5-1.3.02-.47-.14-.94-.5-1.3z"/></defs><g><g class="round" transform="translate(-209 -4126)"><use xlink:href="#up7sa"/></g><g class="arrow" transform="translate(-209 -4126)"><use xlink:href="#up7sb"/></g></g></svg>
                    </span>
                </div>
                <div class="s-blog-slider">
                    <div class="s-blog-item">
                        <div class="s-blog-item__img">
                            <img src="<? bloginfo('template_url' ); ?>/img/blog-1.jpg" alt="alt">
                        </div>
                        <div class="s-blog-item__content">
                            <span class="s-blog-item__date">12 April, 2018</span>
                            <a href="blog-inner.html" class="s-blog-item__head">This is Kuala Lumpur</a>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever.</p>
                            <a href="blog-inner.html" class="read-more">
                                <span class="text">read more</span>
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="13.67" height="13" viewBox="0 0 13.67 13"><defs><path id="1vuqa" d="M749.67 5209.5c0 .37-.13.74-.37 1.04-.1.15-4.93 5-4.93 5-1.22 1.35-3.32-.6-2.14-1.92l1.97-2.37h-6.85c-.92.01-1.37-.88-1.35-1.75-.02-.87.43-1.76 1.35-1.75h6.85l-1.97-2.37c-1.18-1.31.92-3.27 2.14-1.93 0 0 4.83 4.86 4.93 5.01.24.3.37.67.37 1.04z"/></defs><g><g transform="translate(-736 -5203)"><use xlink:href="#1vuqa"/></g></g></svg>
                            </a>
                        </div>
                    </div>
                    <div class="s-blog-item">
                        <div class="s-blog-item__img">
                            <img src="<? bloginfo('template_url' ); ?>/img/blog-2.jpg" alt="alt">
                        </div>
                        <div class="s-blog-item__content">
                            <span class="s-blog-item__date">12 April, 2018</span>
                            <a href="blog-inner.html" class="s-blog-item__head">This is Kuala Lumpur</a>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever.</p>
                            <a href="blog-inner.html" class="read-more">
                                <span class="text">read more</span>
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="13.67" height="13" viewBox="0 0 13.67 13"><defs><path id="1vuqa" d="M749.67 5209.5c0 .37-.13.74-.37 1.04-.1.15-4.93 5-4.93 5-1.22 1.35-3.32-.6-2.14-1.92l1.97-2.37h-6.85c-.92.01-1.37-.88-1.35-1.75-.02-.87.43-1.76 1.35-1.75h6.85l-1.97-2.37c-1.18-1.31.92-3.27 2.14-1.93 0 0 4.83 4.86 4.93 5.01.24.3.37.67.37 1.04z"/></defs><g><g transform="translate(-736 -5203)"><use xlink:href="#1vuqa"/></g></g></svg>
                            </a>
                        </div>
                    </div>
                    <div class="s-blog-item">
                        <div class="s-blog-item__img">
                            <img src="<? bloginfo('template_url' ); ?>/img/blog-3.jpg" alt="alt">
                        </div>
                        <div class="s-blog-item__content">
                            <span class="s-blog-item__date">12 April, 2018</span>
                            <a href="blog-inner.html" class="s-blog-item__head">This is Kuala Lumpur</a>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever.</p>
                            <a href="blog-inner.html" class="read-more">
                                <span class="text">read more</span>
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="13.67" height="13" viewBox="0 0 13.67 13"><defs><path id="1vuqa" d="M749.67 5209.5c0 .37-.13.74-.37 1.04-.1.15-4.93 5-4.93 5-1.22 1.35-3.32-.6-2.14-1.92l1.97-2.37h-6.85c-.92.01-1.37-.88-1.35-1.75-.02-.87.43-1.76 1.35-1.75h6.85l-1.97-2.37c-1.18-1.31.92-3.27 2.14-1.93 0 0 4.83 4.86 4.93 5.01.24.3.37.67.37 1.04z"/></defs><g><g transform="translate(-736 -5203)"><use xlink:href="#1vuqa"/></g></g></svg>
                            </a>
                        </div>
                    </div>
                    <div class="s-blog-item">
                        <div class="s-blog-item__img">
                            <img src="<? bloginfo('template_url' ); ?>/img/blog-3.jpg" alt="alt">
                        </div>
                        <div class="s-blog-item__content">
                            <span class="s-blog-item__date">12 April, 2018</span>
                            <a href="blog-inner.html" class="s-blog-item__head">This is Kuala Lumpur</a>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever.</p>
                            <a href="blog-inner.html" class="read-more">
                                <span class="text">read more</span>
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="13.67" height="13" viewBox="0 0 13.67 13"><defs><path id="1vuqa" d="M749.67 5209.5c0 .37-.13.74-.37 1.04-.1.15-4.93 5-4.93 5-1.22 1.35-3.32-.6-2.14-1.92l1.97-2.37h-6.85c-.92.01-1.37-.88-1.35-1.75-.02-.87.43-1.76 1.35-1.75h6.85l-1.97-2.37c-1.18-1.31.92-3.27 2.14-1.93 0 0 4.83 4.86 4.93 5.01.24.3.37.67.37 1.04z"/></defs><g><g transform="translate(-736 -5203)"><use xlink:href="#1vuqa"/></g></g></svg>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="s-contact s-contact--bg">
        <div class="container">
            <h2 class="h2">Contact Us</h2>
            <div class="row justify-content-center">
                <div class="col-lg-6 no-pad">
                    <div class="s-contact-black s-contact-item s-contact-item--company">
                        <img src="<? bloginfo('template_url' ); ?>/img/envelope.svg" alt="alt" class="envelope">
                        <form class="form-c">
                            <div class="form-c__row">
                                <input type="text" placeholder="Name*">
                            </div>
                            <div class="form-c__row">
                                <input type="text" placeholder="Email*">
                            </div>
                            <div class="form-c__row">
                                <input type="text" placeholder="Phone*">
                            </div>
                            <div class="form-c__row">
                                <input type="text" placeholder="Message*">
                            </div>
                            <div class="centered">
                                <a href="#" class="btn btn--blue btn--company">Submit</a>
                            </div>
                        </form>
                    </div>  
                </div>
            </div>
        </div>
    </section>

<?php get_footer(); ?>